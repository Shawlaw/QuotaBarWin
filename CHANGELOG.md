...
- 托盘小窗重新弹出时不再残留上次的键盘焦点，避免出现异常焦点圈，或按 Enter 误触之前选中的按钮。

## [1.4.1] - 2026-08-24

### Fixed

- 应用更新检查临时失败后会自动再次尝试，避免当天后续检查被跳过。
- 手动检查发现新版本时，主窗口和托盘小窗会及时显示更新提示。
- 主窗口和托盘小窗现在保留可正常显示的最小尺寸，避免缩放过小时界面异常。

## [1.4.0] - 2026-08-24

### Added

- 设置页新增主题选择：可跟随 Windows 系统，或固定使用浅色、深色主题。
- 主窗口与托盘小窗会统一应用所选主题。

### Fixed

- 新增 Provider 完成“保存并测试”后，会立即刷新并显示在概览中，无需等待后台定时刷新。

## [1.3.0] - 2026-08-20

### Added

- 新增本地集成 HTTP API，可直接供本机自动化、脚本、浏览器扩展和其他桌面工具读取额度快照或触发刷新。
- 设置页新增“本地集成 API”：可启停服务、修改端口，并选择监听本机回环地址、多个活动 IPv4/IPv6 网卡或全部活动网卡。

### Security

- 默认关闭；启用后默认只监听 `127.0.0.1`。网络监听可同时保留免 Token 的本机回环地址；启用外部网卡前必须先保存手动填写或随机生成的 Bearer token，未保存令牌时不能保存网络监听设置；已保存令牌以掩码形式显示，可复制、替换或重新生成。
- API 响应不会包含 Provider metadata、诊断信息或敏感错误内容，也不提供配置、Provider 或凭据写入接口。
