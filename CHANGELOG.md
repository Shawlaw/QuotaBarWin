...
- 设置页新增“自动检查应用更新”开关。每天本地时间 08:00 后，用户首次打开主窗口或托盘小窗时会在后台检查一次更新；新安装默认开启，schema 17 升级的既有配置默认关闭。
- 发现更新后，主窗口和托盘小窗都会显示带入场动画的更新提示；点击“前往更新”会打开设置页并定位到“应用更新”区。
- 内部 `update-preview` 构建可通过 `QBWIN_DEMO_APP_UPDATE=1` 体验模拟更新提示，不会联网、下载或改写更新缓存。

### Changed

- 更新提示只保留“前往更新”和“稍后”；“稍后”关闭提示并静默当前版本。
- 下载并重启更新会使用当前提示对应、已经签名校验的候选版本；不会因为官网随后发布了更新版本而在点击下载时切换目标。
- 概览和设置改用独立的内容滚动区，切换页面时各自恢复最后停留的位置。

### Fixed

- 修复从托盘小窗点击“前往更新”时，已打开或从隐藏状态恢复的主窗口可能漏掉导航请求的问题。
- 修复设置页因已处理的更新定位请求在重新打开后重复滚动到应用更新区的问题。
